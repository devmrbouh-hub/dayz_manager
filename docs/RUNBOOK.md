...
2. Удалить `D:\Banov\SERVER_LOCK` (или `{servers[].path}\SERVER_LOCK`).
...
- `GET /api/servers/banov` → `planned_restart.enabled: true`, `next_restart_at` не null
...
- Временный обход: `python scripts/start_banov_direct.py`
