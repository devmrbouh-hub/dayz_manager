...
**Reference (2026-05-24):** OK — planned restart test mode 15 мин, kick на T-5, UI на карточке.
...
**Reference (2026-05-25):** OK — Start → STARTING → READY, Server log, WEAPON filter.
...
**Reference (2026-05-25):** OK — без моргания при открытии лога и во время STARTING.
...
**на reference-хосте (2026-05-23):** WatchDog **сработал**, полный рестарт **не завершился** — `Pre-start mod download failed` (SteamCMD login timeout). Обход: `scripts/start_server_direct.py`.
...
- Junction: `dir D:\Servers\MyServer\@* /AL` (ссылки на !Workshop)
...
**Reference:** не прогонялся (`restart_schedule: []`).
...
2. Убедиться, что RCON работает: `POST /api/servers/server1/rcon/test`.
...
**Reference (2026-05-24):** OK — T-10/T-5 say, lock + kick, restart на T-0; bercon-cli table parse для kick.
...
**Reference:** OK после настройки BattlEye/RCON (2026-05-24).
...
|------|--------|------------|
| UI Restart block | ✅ | ref 2026-05-24 |
| T4b Planned restart | ✅ | ref 2026-05-24 |
| T6 RCON | ✅ | ref 2026-05-24 |
| T1 WatchDog | ⬜ | |
| T2 Mod update / lock | ⬜ | |
| T3 Failed start | ⬜ | |
| T4 CRON (legacy) | ⬜ | |
| T5 Settings hot-reload | ✅ | ref 2026-05-23 |
| T7 RPT + READY | ✅ | ref 2026-05-25 |
| T7b Игровой чат | ✅ | ref 2026-05-25 |
