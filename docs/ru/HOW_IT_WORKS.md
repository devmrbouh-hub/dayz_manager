...
2. `is_running`: PID-файл + путь к exe в каталоге `servers[].path` (при необходимости cwd); не любой процесс DayZ на хосте. При ручном запуске из папки сервера PID может восстановиться. Несколько серверов на одной машине различаются по своему exe.
